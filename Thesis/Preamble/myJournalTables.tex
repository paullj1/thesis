...
\begin{table} \centering
  \tbl{Fault operators used for fault
  injection~\citep{gswfit}.\label{tab:faults}}{%
...
\newcommand{\tabWSWFITFaults}{
\begin{table} \centering
  \tbl{Fault operators used for fault
  injection~\citep{gswfit}.\label{tab:wswfitfaults}}{%
...
\begin{table} \centering
  \tbl{Function entry/exit patterns in IA32 bytecode~\citep{gswfit}.
...
\begin{table} \centering
  \tbl{Function entry/exit patterns in x86-64 bytecode~\citep{gswfit}.
...
\begin{table} \centering
  \tbl{Hypervisor 1 configuration (sandbox/target).\label{tab:hyp1}}{%
...
\begin{table} \centering
  \tbl{Hypervisor 2 configuration (controller).\label{tab:hyp2}}{%
  \begin{tabular}{ | c | l | l | c | l |}
    \hline
    Qty. & Role & Operating System    & CPU / Mem. \\ \hline\hline
...
\newcommand{\tabMessage}{
\begin{table} \centering
  \tbl{Typical authentication message sent as keys that correspond to the
  ...
  \begin{tabular}{ | l | l | }
    \hline
      ...
      EventLogType        & dc.testnet.com                      \\ \hline
...
\begin{table} \centering
  \tbl{Sample message data window after translation.\label{tab:window}}{
  \begin{tabular}{ | l | l | }
    \hline
...
\begin{table} \centering
  \tbl{Classification statistics on test data created before software
  updates.\label{tab:memLeakPreUpdateSVMStats}}{%
  \begin{tabular}{ | l | l | }
    \hline
...
\newcommand{\tabMemLeakPreUpdateSVMConfusionMatrix}{
  \begin{table} \centering
    \tbl{\ac{SVM} test data confusion matrix.
...
\newcommand{\tabMemLeakPreUpdateBoostingConfusionMatrix}{
  \begin{table} \centering
    \tbl{Boosting pre-update test data confusion matrix.
...
\newcommand{\tabMemLeakPostUpdateBoostingSameModelConfusionMatrix}{
  \begin{table} \centering
    \tbl{Boosting post-update old model test data confusion matrix.
...
\newcommand{\tabMemLeakPostUpdateBoostingConfusionMatrix}{
  \begin{table} \centering
    \tbl{Boosting post-update new model test data confusion matrix.
...
\begin{table*} \centering
  \tbl{Cross-validation runs on training data for model selection and
  resulting classification accuracy. \label{tab:model:selection}}{%
...
\begin{table} \centering
  \tbl{Microsoft log message IDs\protect \footnotemark.
  \label{tab:messageIDs}}{%
  \begin{tabular}{ | l | l | }
  \hline
